...
static void treeFactory(GenericTree<int>& tree) {
    ...
    // add the child nodes to the root (8 and 15)
    auto node_8 = root->addChild(8);
    auto node_15 = root->addChild(15);

    // add the child nodes to node_8 (16 and 23)
    auto node_16 = node_8->addChild(16);
    node_8->addChild(23);

    // add the child node to node16 (42)
    node_16->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
    ...
    while (!nodesToExplore.empty()) {
        ...
        // add the data of the current node to results
        results.push_back(currentNode->data);

        // push all children of the current node onto the queue
